...
#flags.DEFINE_string('data_path', '/mydata/AOI2/', 'directory of dataset')
...
flags.DEFINE_string('encoder_model', 'pretrained/encoder.ckpt-20000', 'directory of encoder model')
flags.DEFINE_string('decoder_model', 'pretrained/decoder.ckpt-20000', 'directory of decoder model')
...
def buildExternalDataset ( data_path, batch_size = 16, shuffle = True, crop_size = [None, None], resize = [512, 512], augment = False, buffer = 30):
...
def main(_):
    ...
    with tf.name_scope('loss'):
        ...
        jaccard_index = yy_head / ( tf.reduce_sum(tf.cast(label_placeholder, tf.float32)) + tf.reduce_sum(y_head) - yy_head + 1.0) 
        jaccard_loss = - jaccard_index
    ...
    with tf.name_scope('dataset'):
        ...
        if (FLAGS.use_external):
            train_data, test_data = buildExternalDataset( FLAGS.data_path, batch_size = FLAGS.batch_size,
                                        shuffle = True, crop_size = [512, 512],
                                        augment = True)
        else:
